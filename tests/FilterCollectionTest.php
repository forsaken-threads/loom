...
class FilterCollectionTest extends TestCase
{
    public function testValidConstructors()
    {
        $collection = new FilterCollection([
            'property' => new FilterCriterion('filter-name', 'name'),
        ]);
        foreach ($collection as $property => $filter) {
            $this->assertEquals('property', $property);
            $this->assertEquals(new FilterCriterion('filter-name', 'name'), $filter);
        }
    }
    ...
    {
        try {
            $collection = new FilterCollection([new FilterCriterion('filter-name', 'name')]);
    ...
    {
        $collection = new FilterCollection();
        $collection->addFilter('property', new FilterCriterion('filter-name', 'name'));
        foreach ($collection as $property => $filter) {
            $this->assertEquals('property', $property);
            $this->assertEquals(new FilterCriterion('filter-name', 'name'), $filter);
        }
    ...
    public function testInvalidAddFilter()
    {
        $collection = new FilterCollection();
        try {
            $collection->addFilter('0', new FilterCriterion('filter-name', 'name'));
        } catch (LoomException $e) {
            $this->assertEquals(trans('quality-control.filterable.expected-property', ['property' => 0]), $e->getMessage());
        }
    }
